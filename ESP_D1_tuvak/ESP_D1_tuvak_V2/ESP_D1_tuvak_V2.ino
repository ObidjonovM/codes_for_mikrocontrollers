...
////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////
const String SERIAL_NUMBER = "00112128639d";          // Serial number of the product
const String AP_LOGIN = "FidoElectronics28";       // Access Point (AP) mode default login
const String AP_PASS = "888cbdd1";          // AP mode default password
const String STS_LOGIN = "d0c3ae5d";                 // Login to access settings in AP mode 
const String STS_PASS = "25b8fe8f";                    // Password to access settings in AP mode

...
void getFromDevice() {
  httpResp = sendRequest(slaveServer + END_POINT, "{\"serial_num\":\"" + SERIAL_NUMBER + "\", \"soil_moist\":\"" + currMoist + "\", \"water_poured\":\"" + waterPoured + "\", "+
  +" \"air_humid\":\"" + currHumid + "\", \"temp\":\"" + currTemp + "\", \"slave_server\":\"" + slaveServer + "\", \"measurement_delay\":\"" + measureDelay + "\", "+
  +" \"conn_reist_trails\":\"" + connReistTrails + "\", \"pump_on_delay\":\"" + pumpOnDelay + "\", \"moist_samples\":\"" + moistSamples + "\"}");
...
void deviceSettings(String httpResp) {
  ...
  if (pump_on_delay != "" && pump_on_delay.toInt() != pumpOnDelay) {
    pumpOnDelay = pump_on_delay.toInt();
    mustWriteEEPROM = true;
  }
  if (moist_samples != "" && moist_samples.toInt() != moistSamples) {
...
void loadCredentials() {
  ...
  if (stamp == STAMP) {
    ...
    if (measureStamp == MEASURE_STAMP) {
      ...
      // reading pumpOnDelay value
      startPos += conn_reist_trails.length() + 1;
      String pump_on_delay = readFromEEPROM(startPos);
      pumpOnDelay = pump_on_delay.toInt();

      // reading moistSamples value
      startPos += pump_on_delay.length() + 1;
      String moist_samples = readFromEEPROM(startPos);
      moistSamples = moist_samples.toInt();

      // reading routerLogin value
      startPos += moist_samples.length() + 1;
...
void saveCredentials() {
    ...
    if (measureStamp == MEASURE_STAMP) {
      ...
      // writting pumpOnDelay
      startPos += conn_reist_trails.length() + 1;
      String pump_on_delay = String(pumpOnDelay);
      writeEEPROM(pump_on_delay, startPos);
  
      // writting moistSamples
      startPos += pump_on_delay.length() + 1;
      String moist_samples = String(moistSamples);
      writeEEPROM(moist_samples, startPos);
