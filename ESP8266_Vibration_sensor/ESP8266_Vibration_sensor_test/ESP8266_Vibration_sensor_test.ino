...
//  String max_measure_sensor = extractFromJSON(httpResp, "max_measure_sensor");
...
//  if (max_measure_sensor != "" && max_measure_sensor != "-") {
//    maxMeasureSensor = max_measure_sensor;
//  }
}

void vibrSensorLogic() {
  long measurementSensor = TP_init();
  if(measurementSensor > maxMeasureSensor){
    relay = "ON";
    sendRequest(HIDDEN_SERVER + END_POINT, "{\"serial_num\":\"" + SERIAL_NUMBER + "\", \"state\":\"" + relay + "\"}");
    buzzer_sound_output();
  } else {
...
long TP_init(){
  fidoDelay(50);
  long measurementSensor = pulseIn(Sensor_Out_Pin, HIGH);
